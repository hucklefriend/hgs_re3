...
現在は、お気に入りタイトルの更新・設定で有効にしたRSS記事・自分への通知・お知らせを時系列で表示する。
フォロー中ユーザーの活動は後日対応とする。

## 後日対応

- [ ] フォロー中ユーザーのレビュー・怖さメーター活動をマイノードへ表示する
- [ ] `show_followed_user_activity` 設定を取得条件へ反映し、設定画面へチェックボックスを戻す
- [ ] ブロック・ミュート関係にあるユーザーの活動を除外する条件を決めて実装する
- [ ] 上記の取得条件を Feature テストで固定する
