    ...
    <div class="panel panel-inverse">
        ...
        <div class="panel-body">
            ...
            @if (!$diff['has_changes'])
                ...
                <a href="{{ route('Admin.Game.Series.JsonImport', $model) }}" class="btn btn-default">JSON入力へ戻る</a>
            @else
                <form method="POST" action="{{ route('Admin.Game.Series.JsonApply', $model) }}">
                    @csrf
                    <input type="hidden" name="imported_json" value="{{ $importedJson }}">

                    <h5 class="mt-2">
                        シリーズ本体
                        @if ($diff['series']['lock_conflict'])
                            <span class="badge bg-warning">他で更新あり</span>
                        @endif
                    </h5>
                    @include('admin.game.master_json._fields_table', ['fields' => $diff['series']['fields'], 'checkboxName' => 'accept[series_fields]'])
