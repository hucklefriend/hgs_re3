    ...
    <div class="panel panel-inverse">
        ...
        <div class="panel-body">
            ...
            @if (!$diff['has_changes'])
                ...
                <a href="{{ route('Admin.Game.RelatedProduct.JsonImport', $model) }}" class="btn btn-default">JSON入力へ戻る</a>
            @else
                <form method="POST" action="{{ route('Admin.Game.RelatedProduct.JsonApply', $model) }}">
                    @csrf
                    <input type="hidden" name="imported_json" value="{{ $importedJson }}">

                    @if (!empty($diff['related_product']['fields']))
                        <h5 class="mt-2">
                            関連商品本体
                            @if ($diff['related_product']['lock_conflict'])
        ...
        <div class="panel-footer">
            <a href="{{ route('Admin.Game.RelatedProduct.Detail', $model) }}" class="btn btn-default"><i class="fas fa-arrow-left"></i> 詳細へ戻る</a>
        </div>
    </div>
@endsection
