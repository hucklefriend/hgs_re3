...
export abstract class MainNodeBase extends NodeBase
{
    ...
    public draw(): void
    {
        ...
        this.subLinkNodes.forEach(subLinkNode => {
     ...
     * @param endY 終了点のY座標
     */
    private drawChildCurvedLine(startX: number, startY: number, endX: number, endY: number): void
    {
        console.log(endX, endY);
        const gradient = this.canvasCtx.createLinearGradient(0, 0, 0, this.canvas.height);
        gradient.addColorStop(0, 'rgba(144, 255, 144, 0.5)');   // 薄い緑色
        gradient.addColorStop(1, 'rgba(0, 0, 0, 0.2)');   // 薄い緑色
